...
def stack_data(data, num_shifts, lenT):
	...
	for j in np.arange(num_shifts + 1):
		for count in np.arange(num_traj):
			data_tensor[j, count*newLenT : newLenT + count*newLenT, :] = data[count*lenT + j : count*lenT + j + newLenT,:]
...
def ChooseOptimizer(params, regularized_loss, trainable_var):
	if params['optalg'] == 'adam':
		...
		if params['decay_rate'] > 0:
			optimizer = tf.train.AdadeltaOptimizer(params['lr'], params['decay_rate']).minimize(regularized_loss, var_list=trainable_var)
		else:
			# defaults 0.001, 0.95
		...
		optimizer = tf.train.AdagradDAOptimizer(params['lr'], tf.get_global_step()).minimize(regularized_loss, var_list=trainable_var)
		...
		optimizer = tf.train.ProximalGradientDescentOptimizer(params['lr']).minimize(regularized_loss, var_list=trainable_var)
		...
		if params['decay_rate'] > 0:
			optimizer = tf.train.RMSPropOptimizer(params['lr'], params['decay_rate']).minimize(regularized_loss, var_list=trainable_var)
		else:
...
def TryNet(n, data_train_len, data_test, exp_name, params):
	...
	loss1, loss2, loss3, loss_Linf, loss_L1, loss_L2, loss, regularized_loss = DefineLoss(x, y, g_list, g_list_omega, params, trainable_var)
	...
	num_saved = np.floor((params['num_steps_per_file_pass']/20+1)*data_train_len*params['num_passes_per_file']).astype(int)
...
def savefiles(sess, saver, csvpath, train_test_error, params, weights, biases, endflag):
	...
	print "min train: %.12f, min test: %.12f, min reg. train: %.12f, min reg. test: %.12f" % (params['minTrain'], params['minTest'], params['minRegTrain'], params['minRegTest'])
	save_params(params)
